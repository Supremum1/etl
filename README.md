...
## Текущая версия v4: Native TCP

После v2 выполнены следующие изменения:

- ручные TSV-сериализация и HTTP POST удалены;
- Python использует один `clickhouse-driver.Client`;
- Java использует один Housepower Native JDBC connection;
- батчи кодируются драйверами в сжатые бинарные колонночные блоки;
- ClickHouse принимает их через Native Protocol по TCP `9000`;
- Python читает PostgreSQL через server-side cursor и `fetchmany(fetch_size)`;
- Java читает через потоковый `ResultSet` с `setFetchSize(fetch_size)`.

Результат v4 на тех же 9 759 911 строках и 196 батчах:
...
| Реализация | v2: serialize + load | v4: native load | Изменение |
|---|---:|---:|---:|
...
Полное время при этом не улучшилось: Java стала медленнее v2 примерно на 3.3%, Python на 9.2%, Airflow на 37.1%. Причина находится вне нового ClickHouse load: выросли `extract_ms`, `source_verify_ms`, `overhead_ms`, а у Airflow особенно `prepare_ms`. Следовательно, один последовательный прогон подтверждает пользу Native TCP для стадии загрузки, но не доказывает ускорение всего pipeline.

Java остаётся самой быстрой реализацией v4 и опережает Python примерно на 19% по полному времени, но использует примерно в 4.2 раза больше памяти. Для итогового вывода нужны повторные `transfer`-прогоны на одной подготовленной PostgreSQL-таблице с чередованием порядка запуска и сравнением медианы.
