...
if ($File) { $Common.File = $File }
if ($LimitRows -gt 0) { $Common.LimitRows = $LimitRows }
...
if ($Mode -eq "Transfer") {
  ...
  & "$Root\scripts\run-python.ps1" @Common
  & "$Root\scripts\run-java.ps1" @Common
  & "$Root\scripts\run-airflow.ps1" @Common
}

& "$Root\scripts\query-results.ps1"
